...
def generate_transform(polydata, downsample=[4,4,4,4], method='qslim'):    
                
    mesh = Mesh()
    mesh.SetPolyData(polydata)

    # downsample = list(map( lambda x:int(x), downsample ))
    D, U, F, V = generate_transform_matrices(mesh, downsample, method)

    transform_data = {'down_transform': D, 'up_transform': U, 'face': F, 'vertices':V }
